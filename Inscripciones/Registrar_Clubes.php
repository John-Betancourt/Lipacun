...
<body>
  <?php
    ...
    $Nombre_corto1 = $_POST['Nombre_corto'];
	$Nombre_corto = addslashes($Nombre_corto1);
    $Direccion=$_POST['Direccion'];
	$fecha_inscripcion=date("Y-m-d");
	$estado='3';

    $check_Nombre_query="select * from clubes where nombre_completo_club='$Nombre'";
    $result_check1=$mysqli->query($check_Nombre_query);
    $rows = $result_check1->num_rows;
    $check_Nombre_corto_query="select * from clubes where nombre_corto_club='$Nombre_corto'";
    $result_check2=$mysqli->query($check_Nombre_corto_query);
    $rows1 = $result_check2->num_rows;

    //if ($rows > 0 || $rows1 > 0) {//compara
	if ($rows > 0 ) {//compara
		...
		if ($Reconocimiento==1) {//reconocimiento = si
			$query="insert INTO `clubes` (`departamento`, `municipio`, `nombre_completo_club`, `nombres`, `apellidos`, `identificacion`, `cargo`, `telefono`, `email`, `reconocimiento`, `no_reconocimiento`, `fecha_reconocimiento`, `nombre_corto_club`, `direccion`, `fecha_inscripcion`, `estado`) VALUES ('$Departamento', '$Municipio', '$Nombre', '$Nombres', '$Apellidos', '$CC', '$Cargo', '$Telefono', '$Email', '$Reconocimiento', '$No_reconocimiento', '$Fecha_reconocimiento', '$Nombre_corto', '$Direccion', '$fecha_inscripcion', '$estado')";
		} else {//reconocimiento = no
			$query="insert INTO `clubes` (`departamento`, `municipio`, `nombre_completo_club`, `nombres`, `apellidos`, `identificacion`, `cargo`, `telefono`, `email`, `reconocimiento`, `nombre_corto_club`, `direccion`, `fecha_inscripcion`,`estado`) VALUES ('$Departamento', '$Municipio', '$Nombre', '$Nombres', '$Apellidos', '$CC', '$Cargo', '$Telefono', '$Email', '$Reconocimiento', '$Nombre_corto', '$Direccion', '$fecha_inscripcion', '$estado')";
    	}
