...
<body>
	...
	$query="update `clubes` SET `clave` = '$password_encry', `token` = '$token' WHERE `nombre_corto_club` = '$Nombre_corto_lashes'";
	$result=$mysqli->query($query);
	if ($result) {
		$email     = $row['email'];
		$asunto   = 'LIPACUN - RESTABLECER CONTRASEÑA';
		...
		$mail = new PHPMailer(true);
				...
				try {
					...
					$mail->Subject = $asunto;
					$mail->Body    = '<h3>CLUB Y/O ESCUELA '.$Nombre_corto.'</h3>

					<p>Usted ha solicitado el restablecimiento de su contraseña en Lipacun.</p>
				   ...
				   <p>********************* NO RESPONDER - Mensaje Generado Automáticamente *********************</p>';
					...
					$mail->send();
					$response['response'] = "Se ha restablecido la contrase\u00F1a del club y/o escuela correctamente.";
					$response['type_response'] = 1;
				} catch (Exception $e) {
					$response['response'] = "ERROR PRESENTADO - No se ha podido restablecer la contrase\u00F1a del club y/o escuela, por favor intente nuevamente.";
